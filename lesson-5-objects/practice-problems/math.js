...
so nearly all languages use radians for circle and arc calculations.

Use the Math.PI property to create a function that converts radians to
degrees.
*/
...
console.log(radiansToDegrees(Math.PI / 6)); // ~30
...
variable to the wrong value. You can use the Math.abs method to simplify
matters by forcing a value positive. For this problem, create a variable with a
value of -180, then use Math.abs to log the positive value of the variable.
*/

const exampleNum = -180;

const alwaysPositive = (number) => {
  console.log(Math.abs(number));
}

alwaysPositive(exampleNum);
...
makes this easier than it would otherwise be. Use this method to find the
square root of 16777216.
*/
...
less than 0.5, Math.round rounds the value downwards; otherwise, it rounds the
value upwards. Note: if the fractional part is exactly 0.5, Math.round rounds
upwards. Math.floor rounds any number downward to the next lower integer.
...
console.log(randomFromRange(10, 1));
console.log(randomFromRange(1, 1));
